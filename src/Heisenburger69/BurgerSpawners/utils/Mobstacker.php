...
/**
 * Class Mobstacker
 * @package Heisenburger69\BurgerSpawners\utils
 */
class Mobstacker
{
    ...
    public function findNearStack(int $range = 16): ?Living
    {
        ...
        foreach ($entity->getLevel()->getNearbyEntities($boundingBox) as $nearbyEntity) {
            if (!$nearbyEntity instanceof Player and $nearbyEntity instanceof Living) {
                if ($entity->distance($nearbyEntity) <= $range and $entity->getSaveId() === $nearbyEntity->getSaveId() && !in_array(strtolower($entity->getSaveId()), Main::$instance->exemptedEntities)) {
